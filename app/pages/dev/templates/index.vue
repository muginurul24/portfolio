...
function askDelete(row: DevTemplate) {
...
async function doDelete() {
  ...
  try {
    await $fetch(`/api/dev/templates/${pendingDelete.value.id}`, { method: 'DELETE' })
...
<template>
  ...
  <DevConfirmModal
    ...
    :title="t('dev.catalog.confirmDeleteTemplate', { name: pendingDelete?.label || '' })"
    color="error"
    :loading="!!deletingId"
    @confirm="doDelete"
  />
